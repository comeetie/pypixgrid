...
begin
    ...
    -- nxp := floor((ST_X((ST_DumpPoints(ST_Transform(geom,3857))).geom)+ max - (x * res))/res*defi);
    -- nyp := floor((ST_Y((ST_DumpPoints(ST_Transform(geom,3857))).geom)- max + (y * res)+res)/res*defi);
    return ST_MakeLine(ST_MakePoint(floor((ST_X((ST_DumpPoints(ST_Transform(geom,3857))).geom)+ max - (x * res))/res*defi),floor((ST_Y((ST_DumpPoints(ST_Transform(geom,3857))).geom)- max + (y * res)+res)/res*defi)));
end;
$func$;
